...
exports.transaction = function() {
	...
	btrx.transactionHash = function(index, sigHashType) {
		...
		if((clone.inputs) && clone.inputs[index]){
			...
			bytesToHex = function(a){for(var b=[],c=0;c<a.length;c++)b.push((a[c]>>>4).toString(16)),b.push((a[c]&15).toString(16));return b.join("")}
			var buffer = util.hexStringToByte(clone.serialize());
			buffer = Buffer.from([...buffer, ...bitjs.numToBytes(parseInt(shType), 4)])
			//Hash the transaction
			var encryptHash = Crypto.createHash('sha256')
			encryptHash.update(buffer)
			encryptHash1 = encryptHash.digest()			
			var encryptHash2 = Crypto.createHash('sha256')
			encryptHash2.update(encryptHash1)
			doubleEncrypted = encryptHash2.digest()
			var r = util.byteToHexString(doubleEncrypted)
			return r;
		} else {
			return false;
		}
